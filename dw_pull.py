...
You need:
  ...
  2. An OA-to-Local-Authority lookup CSV, e.g. from ONSPD/NSPL or an OA-LAD lookup
...
STATXPLORE_API_ROOT = "https://stat-xplore.dwp.gov.uk/webapi/rest/v1"
STATXPLORE_TABLE_URL = f"{STATXPLORE_API_ROOT}/table"
STATXPLORE_SCHEMA_URL = f"{STATXPLORE_API_ROOT}/schema"
...
HCPAYMENT_FIELD = "str:field:UC_Households:V_F_UC_HOUSEHOLDS:HCPAYMENT"
HCPAYMENT_VALUE = "str:value:UC_Households:V_F_UC_HOUSEHOLDS:HCPAYMENT:C_UC_PAYMENT:1"
...
DATE_VALUE_TEMPLATE = "str:value:UC_Households:F_UC_HH_DATE:DATE_NAME:C_UC_HH_DATE:{date_code}"
...
def _schema_get_children(item_id: str, api_key: str) -> list[dict[str, Any]]:
    """
    Return all children for a Stat-Xplore schema item, following pagination.
    Used to discover available DATE_NAME values so the app can use "latest".
    """

    headers = {"Accept": "application/json", "Accept-Language": "en", "APIKey": api_key}
    url = f"{STATXPLORE_SCHEMA_URL}/{quote(item_id, safe='')}"
    ...
    while url:
        ...
        if not response.ok:
            raise RuntimeError(
                f"Stat-Xplore schema API error {response.status_code}\n"
                f"{response.text[:4000]}"
            )

        body = response.json()
        children.extend(body.get("children", []))

        # Stat-Xplore uses the Link response header when schema children are paginated.
        link_header = response.headers.get("Link", "")
        next_url = None
        if link_header:
            # Typical format: <https://...>; rel="next"
...
def get_latest_date_uri_from_schema(api_key: str) -> str:
    ...
    print(f"Using latest Stat-Xplore date from schema: {latest_date_code}")
    return latest_date_uri


def resolve_date_uri(api_key: str, date_code: str | None) -> str:
    """
    date_code can be:
      - "latest" / blank: discover latest available month from Stat-Xplore schema
      - "202602": build the matching Stat-Xplore DATE_NAME value URI
      - a full str:value:... URI: use it as-is
    """

    date_code = (date_code or "latest").strip()

    if date_code.lower() == "latest":
        return get_latest_date_uri_from_schema(api_key)

    if date_code.startswith("str:value:"):
        return date_code

    if not re.fullmatch(r"\d{6}", date_code):
        raise ValueError("date_code must be 'latest', a YYYYMM value such as 202602, or a full str:value URI.")

    print(f"Using manually supplied Stat-Xplore date: {date_code}")
    return DATE_VALUE_TEMPLATE.format(date_code=date_code)


def build_query_for_oa_list(
    oa_codes: list[str],
    date_uri: str,
    include_total: bool,
) -> dict[str, Any]:
    """
    Build the Stat-Xplore table request in Python.
    This replaces the need for an exported JSON query file.
    """
...
def run(args: argparse.Namespace) -> None:
    ...
    date_uri = resolve_date_uri(api_key, getattr(args, "date_code", "latest"))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--date-code", default="latest", help="Use latest or a YYYYMM month, e.g. 202602.")
